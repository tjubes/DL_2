...
def train_one_epoch_batched(model, 
    ...
    for batch_data, batch_labels in load_and_normalize_files_batch(
        ...
        train_data, val_data, train_labels, val_labels = train_test_split(
            batch_data, batch_labels, test_size=0.2, random_state=42, stratify=batch_labels
        )

        # === Train ===
        train_dataset = MEGDataset(train_data, train_labels, label_map)
        ...
        # === Validation ===
        model.eval()
        val_dataset = MEGDataset(val_data, val_labels, label_map)
        ...
        # Clear memory
        del train_dataset, val_dataset, train_loader, val_loader, batch_data, batch_labels
...
def train_cross_subject_model(filepath, 
    ...
    for epoch in range(epochs):
        train_loss, train_acc, val_loss, val_acc = train_one_epoch_batched(
        ...
        if early_stopper.best_score is None or val_acc >= early_stopper.best_score:
    ...
    print(f"\n🌟 TRAINING COMPLETED")
    print(f"Best validation accuracy: {early_stopper.best_score:.4f} ({early_stopper.best_score*100:.1f}%)")
...
def analyze_overfitting(train_losses, train_accs, val_losses, val_accs, all_results):
    """
    ...
    if test_accuracies:
        ...
        val_gap = final_val_acc - avg_test_acc

        print(f"Final training accuracy:    {final_train_acc:.4f}")
        print(f"Final validation accuracy:  {final_val_acc:.4f}")
        print(f"Average test accuracy:      {avg_test_acc:.4f}")
        print(f"Train-test gap:             {train_gap:.4f}")
        print(f"Val-test gap:               {val_gap:.4f}")
...
def run_complete_cross_subject_experiment(filepath, model_fn, label_map,
    ...
    # Train the model
    model, train_losses, train_accs, val_losses, val_accs = train_cross_subject_model(
        filepath=filepath,
        model_fn=model_fn,
        LABEL_MAP=label_map,
        epochs=epochs,
    ...
    # Analyze overfitting
    analyze_overfitting(train_losses, train_accs, val_losses, val_accs, test_results)

    return model, train_losses, train_accs, val_losses, val_accs, test_results
